...
func (c *Client) ImportJob(j ImportParams) (string, error) {
	...
	res, postErr := c.httpPost("jobs/import", withBody(bytes.NewReader(data)), requestXML(), queryParams(opts))
	if postErr != nil {
		return "", postErr
	}
...
func (c *Client) FindJobByName(name string, project string) (*JobDetails, error) {
	...
	if len(jobs.Jobs) > 0 {
		for _, d := range jobs.Jobs {
			if d.Name == name {
				joblist, joblistErr := c.GetJob(d.ID)
				if err != nil {
